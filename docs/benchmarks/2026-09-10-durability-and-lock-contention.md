...
| Segment preallocation fixed that | **3.1x** on durable writes, sync p99 **12-15x** tighter |
| KDB now **beats MongoDB** on durable writes and reads | 1.25-1.6x, both over TCP, both in 2 vCPU / 1 GiB containers |
| Expiry took an `RWMutex` on every point read | removing it: **+97/+100%** on contended reads |
| `dag.Pin` held 93.9% of all mutex delay | removing it: **no measurable change** - see §7 |
...
Single writer, `-benchtime 2000x -count=5`, medians:

| Mode | macOS | Linux container |
|---|---:|---:|
| `sync-full` | 4,098 µs | 464 µs |
| `sync-fast` | 367 µs | **579 µs** |
| `async-100ms` | 38.6 µs | 35.9 µs |

`sync-fast` is an 11x win on macOS and measured *slower* than `sync-full` on Linux - consistently,
5/5 samples at both 1 and 2 vCPU (564,659 vs 570,677 ns at 1; 567,571 vs 568,627 at 2).
...
2 vCPU / 1 GiB, `GOMAXPROCS=2`, medians of 5:
...
**Tightening the interval is free.** 1 ms costs no more than 100 ms, because a sustained writer's
...
**3.1x** on the best configuration, with the sync p99 **12-15x** tighter - the millisecond tail was
metadata-journal contention, and it is gone.
...
**Writes, durable tier** (KDB `sync`, Mongo `j:true`):
...
| MongoDB `j:false` | 48,835 | - | 19,751 | - |

*before/after = preallocation off/on; Mongo is unchanged because it already preallocates.*

**What each layer costs:**

- **Reads: the transport is essentially the entire cost.** In-process ~300 ns against ~34 µs over
  TCP - about **100x**. The storage engine is sub-microsecond; the wire is everything. If a
  consumer can embed, that is not a tuning decision.
- **Writes: the transport is nearly free** - ~20% - because the fsync dominates it completely.
- **gRPC costs ~1.4x over raw TCP on reads and ~nothing on writes.** HTTP/2 framing shows up
  exactly where messages are small and latency-bound, and disappears where the disk dominates.
...
| | KDB | MongoDB | |
|---|---:|---:|---|
| Durable writes, par=2 | 138,673 | 172,861 | **KDB 1.25x** |
| Durable writes, par=16 | 56,870 | 86,682 | **KDB 1.5x** |
| Reads, par=2 | 34,177 | 52,728 | **KDB 1.5x** |
| Reads, par=16 | 11,435 | 18,643 | **KDB 1.6x** |
...
2. KDB's *default* is a stronger guarantee than Mongo's. Mongo's `j:false` (48.8 µs) is still
   ~2.8x faster than KDB's `sync` (138.7 µs) - a different promise, not a better engine. The
...
| Row (ops/sec) | before | after | pass 1 | pass 2 |
|---|---:|---:|---:|---:|
...
| Mutex delay, memory-only writes | before | after |
|---|---:|---:|
| Total, all locks | 3,370 ms | **605 ms** (-82%) |
| `dag.Pin` | 3,166 ms (93.9%) | **302 ms** (10.5x less) |

| Throughput (ops/sec) | before | after | pass 1 | pass 2 |
|---|---:|---:|---:|---:|
...
**No measurable change.** The reasoning error is worth recording:
...
budget is exceeded, which under memory-only writes is often. A full 180-row matrix afterwards was
green (no failures, no `deadline exceeded`) and showed nothing alarming, but it was run against a
different baseline on a different machine state and cannot attribute a 1-2% delta to anything - the
back-to-back A/B is the trustworthy measurement here, and it says this costs a little and buys
nothing measurable.
...
- **The capacity-1 write gate.** Measured: KDB `async` throughput is *flat* from 2 to 16 concurrent
  writers (27.3 µs -> 27.6 µs) while Mongo scales 2.7x. This has evidence behind it, unlike §7.2,
  and is the standing suspect for what actually bounds memory-only writes.
