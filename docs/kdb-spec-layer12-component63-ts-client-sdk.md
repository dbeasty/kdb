...
### 2.3 The hard prerequisite: a WebSocket server

`go/kdb/transport/ws/transport.go:128` returns `501 websocket upgrade not implemented`. The Go
side has a WebSocket **client** but no WebSocket **server**.

This matters more than it looks. Per the finish-up plan's fixed decisions, the Go service is the
production deployment target ("zero JVM processes"); a browser can only speak WebSocket. So today
the intersection of "production server" and "browser-reachable" is empty. A Node client can
connect over TCP right now; a browser cannot connect to production at all.

**Therefore: the Go WS listener (finish-up plan item 4.G) is a prerequisite for Phase 1's browser
half, and should be scheduled ahead of it.** The Node half has no such prerequisite and can
proceed in parallel. This spec does not attempt to work around the gap — a client-side workaround
for a missing server listener does not exist.
...
1. Go WebSocket server listener (§2.3) — prerequisite, and not this component's code.
...
Steps 2–5 are **done**, in `packages/kdb-client` — the wire layer, both transports, the full
operation set, and 68 passing tests. Fixtures are generated by `go/cmd/kdb-tsfixtures` from the
real Go encoder and cover frames, envelopes, byte arrays, transaction encoding and content
hashes; the TS canonical encoder reproduces Go's `DocumentBody` bytes and SHA-256 exactly.
