...
// BenchmarkFileBackedUpsertModes compares the durability/sync-mode matrix on
// the same end-to-end path as BenchmarkFileBackedUpsert (which stays the
// default-config baseline). This is the benchmark behind the Mongo-parity
// write work: sync+full pays a physical F_FULLFSYNC before acking (~4ms on
// Apple SSDs, the honest power-loss-proof cost), sync+fast pays a barrier
// sync instead, and async acks from memory with interval journal flushes -
// MongoDB's default write-concern model.
func BenchmarkFileBackedUpsertModes(b *testing.B) {
	modes := []struct {
		name string
		opts embed.StorageOptions
	}{
		{"sync-full", embed.StorageOptions{Durability: storage.DurabilitySync, SyncMode: storio.SyncModeFull}},
		{"sync-fast", embed.StorageOptions{Durability: storage.DurabilitySync, SyncMode: storio.SyncModeFast}},
