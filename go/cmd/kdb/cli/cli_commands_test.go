...
func TestParseEveryCommandForm(t *testing.T) {
	for _, tc := range []struct {
		...
		// A bare log is bounded by default: walking a million-commit
		// history because no flag was passed is a surprise, and --limit is
		// right there.
		{"log", []string{"log", "app/data"}, LogCmd{Namespace: "app/data", Limit: defaultLogLimit}},
		{"log limit", []string{"log", "app/data", "--limit", "5"}, LogCmd{Namespace: "app/data", Limit: 5}},
		{"log oneline", []string{"log", "app/data", "--oneline"},
			LogCmd{Namespace: "app/data", Limit: defaultLogLimit, Oneline: true}},
		{"log skip", []string{"log", "app/data", "--skip", "2", "--limit", "3"},
			LogCmd{Namespace: "app/data", Limit: 3, Skip: 2}},
		{"get at", []string{"get", "app/data", "doc1", "--at", "head~2"},
			GetCmd{Namespace: "app/data", DocID: "doc1", At: "head~2"}},
		{"show", []string{"show", "app/data", "head~1"}, ShowCmd{Namespace: "app/data", Revision: "head~1"}},
		{"diff", []string{"diff", "app/data", "head~1", "head"},
			DiffCmd{Namespace: "app/data", From: "head~1", To: "head"}},
		{"revert", []string{"revert", "app/data", "head~1"},
			RevertCmd{Namespace: "app/data", Revision: "head~1"}},
		{"tag list", []string{"tag", "list", "app/data"}, TagListCmd{Namespace: "app/data"}},
		{"tag create", []string{"tag", "create", "app/data", "v1"},
			TagCreateCmd{Namespace: "app/data", Name: "v1", Revision: "head"}},
		{"tag create at rev", []string{"tag", "create", "app/data", "v1", "head~3", "first", "release"},
			TagCreateCmd{Namespace: "app/data", Name: "v1", Revision: "head~3", Message: "first release"}},
		{"tag delete", []string{"tag", "delete", "app/data", "v1"},
			TagDeleteCmd{Namespace: "app/data", Name: "v1"}},
